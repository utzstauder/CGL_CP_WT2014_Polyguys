...
public abstract class AbstractTargetFollower : MonoBehaviour
{
	...
	void FixedUpdate() {
		...
		if (updateType == UpdateType.FixedUpdate || updateType == UpdateType.Auto && target != null && (target.GetComponent<Rigidbody2D>() != null && !target.GetComponent<Rigidbody2D>().isKinematic)) {
	...
	void LateUpdate() {
		...
		if (updateType == UpdateType.LateUpdate || updateType == UpdateType.Auto && target != null && (target.GetComponent<Rigidbody2D>() == null || target.GetComponent<Rigidbody2D>().isKinematic)) {
			FollowTarget(Time.deltaTime);
		}
	}
